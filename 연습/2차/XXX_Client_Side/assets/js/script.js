const Render = class {
    ...
    setCartInfo () {
        ...
        for (const key in cart) {
            ...
            const text = `
                <tr>
                    ...
                    <td class="albumprice">
                        ￦ ${numberFormat(albumData.price.replace('원', ''))}
                    </td>
                    <td class="albumqty">
                        <input type="number" data-idx="${key}" class="form-control" value="${data.quantity}">
                    </td>
                    <td class="pricesum">
                        ￦ ${numberFormat(data.totalPrice)}
                    </td>
                    <td>
                        <button class="btn btn-default" data-idx="${key}">
                            <i class="fa fa-trash-o"></i> 삭제
                        </button>
                    </td>
                </tr>
            `;
            $('.modal-body tbody').append(text);
        }
        ...
        $('.modal-body .totalprice span').text(`￦${numberFormat(totalPrice)}`);
    }
    ...
    appendList (key = null) {
        ...
        const sortList = musicList.sort(function (a, b) {
            const aDate = a.release.replace(/\./gi, "");
            const bDate = b.release.replace(/\./gi, "");
            return bDate - aDate;
        });
        const category = $('#main-menu a.active-menu').data('category');
        $('h2').text(category);
        for (const data of sortList) {
            if (category !== 'ALL' && category !== data.category) continue;
            let albumName = data.albumName;
            let artist = data.artist;
            if (key !== null && albumName.indexOf(key) === -1 && key !== null && artist.indexOf(key) === -1) continue;
            if (key !== null) {
                const reg = new RegExp(key, 'gi');
                albumName = albumName.replace(reg, `<strong style="background:#09f;color:#fff">${key}</strong>`);
                artist = artist.replace(reg, `<strong style="background:#09f;color:#fff">${key}</strong>`);
            }
            const thisCart = this.model.cart[data.idx];
            const cartState = thisCart ? `추가하기 (${thisCart.quantity}개)` : '쇼핑카트담기';
            const text = `
                    ...
                    <div class="product-items">
                        ...
                        <div class="produ-cost">
                            ...
                            <span>
                                ...
                                <p>￦${numberFormat(data.price.replace('원', ''))}</p>
                            </span>
                            <span class="shopbtn">
        ...
        if (key !== null && target.find('.product-grid').length == 0) {
            target.append('<h3>검색된 앨범이 없습니다.</h3>');
        }
    }
    ...
    categorySelect () {
        const $render = this
        return function () {
            $("#main-menu a.active-menu").removeClass('active-menu');
    ...
    search () {
        const $render = this
        return function () {
            const key = $(this).parents('.form-group').find('input').val();
    ...
    cartInsert () {
        ...
        return function () {
            ...
            const albumData = model.musicList[idx];
            let quantity = 1;
            if (model.cart[idx] !== undefined) {
                quantity += model.cart[idx].quantity;
            }
            const totalPrice = albumData.price.replace('원','')*quantity;
            const data = {
                'quantity':quantity,
    ...
    cartUpdate () {
        ...
        return function () {
            ...
            model.cart[idx].totalPrice = albumData.price.replace('원','')*quantity;
            model.setCart();
            $render.setCartInfo();
            $render.appendList();
        }
